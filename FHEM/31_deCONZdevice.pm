...
my $deCONZdevice_hasDataDumper = 1;

sub deCONZdevice_Initialize($)
{
  my ($hash) = @_;
  ...
  #Consumer
  $hash->{DefFn}    = "deCONZdevice_Define";
  $hash->{UndefFn}  = "deCONZdevice_Undefine";
  $hash->{SetFn}    = "deCONZdevice_Set";
  $hash->{GetFn}    = "deCONZdevice_Get";
  $hash->{AttrFn}   = "deCONZdevice_Attr";
  $hash->{AttrList} = "IODev ".
                      "delayedUpdate:1 ".
                      "ignoreReachable:1,0 ".
                      "realtimePicker:1,0 ".
                      "color-icons:1,2 ".
                      "transitiontime ".
                      "model:".join(",", sort map { $_ =~ s/ /#/g ;$_} keys %deviceModel)." ".
                      "setList:textField-long ".
                      "subType:extcolordimmer,colordimmer,ctdimmer,dimmer,switch ".
                      $readingFnAttributes;

  #$hash->{FW_summaryFn} = "deCONZdevice_summaryFn";

  FHEM_colorpickerInit();

  eval "use Data::Dumper";
  $deCONZdevice_hasDataDumper = 0 if($@);
}

sub
deCONZdevice_devStateIcon($)
{
  my($hash) = @_;
...
deCONZdevice_summaryFn($$$$)
{
  ...
  return deCONZdevice_devStateIcon($hash);
}

sub deCONZdevice_Define($$)
{
  my ($hash, $def) = @_;
  ...
  return "Usage: define <name> deCONZdevice [group|sensor] <id> [interval]"  if(@args < 3);
  ...
  if( !$hash->{helper}->{devtype} ) {
    ...
    $attr{$name}{devStateIcon} = '{(deCONZdevice_devStateIcon($name),"toggle")}' if( !defined( $attr{$name}{devStateIcon} ) );
  ...
  if( $init_done ) {
    deCONZdevice_GetUpdate($hash);
  } else {
    InternalTimer(gettimeofday()+10, "deCONZdevice_GetUpdate", $hash, 0);
  }

  return undef;
}

sub deCONZdevice_Undefine($$)
{
  my ($hash,$arg) = @_;
...
deCONZdevice_Set($@)
{
  ...
  if( $hash->{helper}->{devtype} eq 'G' ) {
    if( $cmd eq 'lights' ) {
      ...
      my $obj = { 'lights' => deCONZ_string2array($args[0]), };

      my $result = deCONZdevice_ReadFromServer($hash,$hash->{ID},$obj);
      if( $result->{success} ) {
        RemoveInternalTimer($hash);
        deCONZdevice_GetUpdate($hash);
      }
      ...
      my $result = deCONZdevice_ReadFromServer($hash,"$hash->{ID}/action",$obj);
      return $result->{error}{description} if( $result->{error} );
      ...
      if( $hash->{helper}->{update_timeout} == -1 ) {
        ...
        RemoveInternalTimer($hash);
        InternalTimer(gettimeofday()+$hash->{helper}->{update_timeout}, "deCONZdevice_GetUpdate", $hash, 0);
      } else {
        RemoveInternalTimer($hash);
        deCONZdevice_GetUpdate( $hash );
      }
    ...
    if( $cmd eq "statusRequest" ) {
      ...
      return deCONZ_Set( $shash, $shash->{NAME}, 'setsensor', $id, @args );

      return undef;

    } elsif( my @match = grep { $cmd eq $_ } keys %{($hash->{helper}{setList}{cmds}?$hash->{helper}{setList}{cmds}:{})} ) {
      return deCONZ_Set( $shash, $shash->{NAME}, 'setsensor', $id, $hash->{helper}{setList}{cmds}{$match[0]} );
      ...
      foreach my $entry (@{$entries}) {
        if( join(' ', @aa) =~ /$entry->{regex}/ ) {
          ...
          $json =~ s/\$2/$VALUE2/;
          $json =~ s/\$3/$VALUE3/;
          return deCONZ_Set( $shash, $shash->{NAME}, 'setsensor', $id, $json );
  ...
  if( (my $joined = join(" ", @aa)) =~ /:/ ) {
    ...
    for( my $i = 0; $i <= $#cmds; ++$i ) {
      deCONZdevice_SetParam($name, \%obj, split(" ", $cmds[$i]) );
    }
  } else {
    ...
    deCONZdevice_SetParam($name, \%obj, $cmd, $value, $value2);
  }
...
#  }
  ...
  if( scalar keys %obj ) {
    ...
    if( $hash->{helper}->{devtype} eq 'G' ) {
      $hash->{helper}->{update} = 1;
      $result = deCONZdevice_ReadFromServer($hash,"$hash->{ID}/action",\%obj);
    } else {
      $result = deCONZdevice_ReadFromServer($hash,"$hash->{ID}/state",\%obj);
    }
...
sub
deCONZdevice_GetUpdate($)
{
  my ($hash) = @_;
  my $name = $hash->{NAME};

  if( $hash->{helper}->{devtype} eq 'G' ) {
    my $result = deCONZdevice_ReadFromServer($hash,$hash->{ID});
    ...
    deCONZdevice_Parse($hash,$result);
  ...
  deCONZdevice_Parse($hash,$result);
  deCONZ_updateGroups($hash->{IODev}, $hash->{ID}) if( $hash->{IODev}{TYPE} eq 'deCONZ' );
}
...
deCONZdeviceSetIcon($;$)
{
  ...
  if( $hash->{modelid} ) {
    my $model = $deviceModel{$hash->{modelid}};
...
deCONZdevice_Parse($$)
{
  ...
  if( ref($result) ne "HASH" ) {
    if( ref($result) && $deCONZdevice_hasDataDumper) {
  ...
  Log3 $name, 5, Dumper $result if($deCONZdevice_hasDataDumper);
  ...
  if( $hash->{helper}->{devtype} eq 'S' ) {
    ...
    if( my $state = $result->{state} ) {
      ...
      Log3 $name, 5, "$name: ". Dumper $result if($deCONZdevice_hasDataDumper);
      ...
      #$readings{battery} = $state->{lowbattery}?'low':'OK' if( defined($state->{lowbattery}) and !defined($readings{battery}) );
    }
  ...
  if( !defined($attr{$name}{subType}) ) {
    if( defined($attr{$name}{model}) ) {
      if( defined($deviceModel{$attr{$name}{model}}{subType}) ) {
        $attr{$name}{subType} = $deviceModel{$attr{$name}{model}}{subType};

        deCONZdeviceSetIcon($hash) if( $hash->{helper}{fromAutocreate} );
    ...
    $attr{$name}{subType} = $deviceModel{$attr{$name}{model}}{subType} if( defined($deviceModel{$attr{$name}{model}}{subType}) );
  }
...
=pod
=item summary    devices connected to a deCONZ module
=item summary_DE Geräte an einem deCONZ module
=begin html

<a name="deCONZdevice"></a>
<h3>deCONZdevice</h3>
<ul>
  <br>
  <a name="deCONZdevice_Define"></a>
  <b>Define</b>
  <ul>
    <code>define &lt;name&gt; deCONZdevice [group|sensor] &lt;id&gt; [&lt;interval&gt;]</code><br>
    <br>

    Defines a device connected to a <a href="#deCONZ">deCONZ</a>.<br><br>
    ...
    Examples:
    <ul>
      <code>define bulb deCONZdevice 1</code><br>
      <code>define LC deCONZdevice 2</code><br>
      <code>define allLights deCONZdevice group 0</code><br>
    </ul>
  </ul><br>

  <a name="deCONZdevice_Readings"></a>
  <b>Readings</b>
  <ul>
    <li>bri<br>
